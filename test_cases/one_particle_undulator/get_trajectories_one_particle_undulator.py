...
y          = r_beta * sin(omega_beta * t_ret)    # y positions of the electron electron
vy         = np.zeros(np.size(y))                # y velocity of electrons - prealocate

for i in range(1,len(vy)):
  vy[i]    = (y[i]-y[i-1])/dt_ret                # calculate velocity from y-positions

vy[0]    = vy[1]                                          # change the initial velocity to next time step velocity  
