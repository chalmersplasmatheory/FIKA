...
# Other parameters used were E_slice_eV = 0.1, t_slice_s = 1e-18, E_radMax_eV = 200.
fig, ax = plt.subplots()
...
fig, ax = plt.subplots()
plt.plot(ene, spectrum_ene / 1e-15) 
plt.title(r'$E_{e} = $ 25 MeV, $r_{\beta}$ = 0.5 $\rm{\mu m}$',fontsize=14)
ax.tick_params(axis="x",direction="in", bottom=True, top=True, left=True, right=True)
ax.tick_params(axis="y",direction="in", bottom=True, top=True, left=True, right=True) 
plt.xlabel('$E_{ph}$ [eV]', fontsize=14)
plt.ylabel(r'$\frac{d^2 W}{d\omega d\Omega}$ [$10^{-15}$ $\rm{J}$ $\rm{eV}^{-1}$ $\rm{sr}^-1$]', fontsize=14)
plt.xticks(fontsize = 14)
plt.yticks(fontsize = 14)
plt.xlim([0,300])
plt.ylim([0,0.04])
